...
public class DGNfile implements AutoCloseable {
	 ...
	 * return spatial elements (elements with a known bounding box) and only those
	 * elements for which this bounding box overlaps the requested region.
	 *
	 * If all four values (dfXMin, dfXMax, dfYMin and dfYMax) are zero, the spatial
	 * filter is disabled. Note that installing a spatial filter won't reduce the
	 * amount of data read from disk. All elements are still scanned, but the amount
	 * of processing work for elements outside the spatial filter is minimized.
	 *
	 ...
	 * reasons as well. If a failure occurs, a value of -1 is returned.
	 * 
	 * @return
	 */
	public long tell() {
	...
	/**
	 * Função que faz a leitura de <code>size</code> * <code>blocks</code> bytes a
	 * partir de <code>offSet</code> e os aloca em {@link DGNfile#abyElem}
	 * 
	 * @param offSet
	 * @param size
	 *               <ol>
	 ...
	 * 
	 * This function returns a value of zero if the operation was successful, and a
	 * nonzero value to indicate failure. A successful call also clears the
	 * end-of-file indicator of stream and discards any characters that were
	 * ``pushed back'' by the use of ungetc.
	 * 
	 * fseek either flushes any buffered output before setting the file position or
	 * else remembers it so it will be written later in its proper place in the
	 * file.
	 * 
	 * @param offset
	 * @return
	 ...
	 *
	 * Rewind the indicated DGN file, so the next element read with DGNReadElement()
	 * will be the first. Does not require indexing like the more general
	 * DGNReadElement() function.
	 *
	 */
	public void rewind() {
		seek(0);
	 ...
	 * Open a DGN file.
	 *
	 * The file is opened, and minimally verified to ensure it is a DGN (ISFF) file.
	 * If the file cannot be opened for read access an error with code
	 ...
	 *
	 * If successful a handle for further access is returned. This should be closed
	 * with DGNClose() when no longer needed.
	 *
	 ...
	 *
	 * @return handle to use for further access to file using DGN API, or NULL if
	 *         open fails.
	 */
